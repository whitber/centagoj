...
from pyclickup.models import Task, Team, User, Comment
...
class ClickUp:
    ...
    def _req(
        self, path: str, method: str = "get", version: int = 2, **kwargs: Any
    ) -> Response:
        """requests wrapper"""
    ...
    def get(
        self, path: str, raw: bool = False, version: int = 2, **kwargs: Any
    ...
    def delete_task(self, task_id: str) -> bool:
        ...
        @ret success
        @link api:
            https://jsapi.apiary.io/apis/clickup20/reference/0/tasks/delete-task.html
        """
        try:
    ...
    def complete_task(self, task_id: str) -> bool:
        try:
            body = {"status": "closed"}
            self.put("task/{}".format(task_id), version=2, json=body)
            print("tried completing the task")
            return True
        except Exception:
            self._log("failed to complete task `{}`".format(task_id))
            return False

    def get_task_comments(self, task_id: str):
        try:
            resp = self.get("task/{}/comment".format(task_id), version=2)
            if not isinstance(resp, dict):
                return []
            return [Comment(x, client=self) for x in resp['comments']]
        except Exception:
            self._log("failed to get comments for task `{}`".format(task_id))
            return None

    def add_comment_to_task(self, task_id: str, comment_text: str) -> bool:
        try:
            body = {"comment_text": comment_text}
            self.post("task/{}/comment".format(task_id), version=2, json=body)
            return True
        except Exception:
            self._log(
                "failed to add the comment `{}` to task `{}`".format(
                    comment_text, task_id
                )
            )
            return False
    ...
    def add_tag_to_task(self, task_id: str, tag: str) -> bool:
        try:
            self.post("/task/{}/tag/{}".format(task_id, tag), version=2)
            return True
        except Exception:
            self._log("failed to add the tage `{}` to task `{}`".format(tag, task_id))
            return False
