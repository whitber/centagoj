...
from pyclickup.utils.text import ts_to_timedelta
...
class CustomField(BaseModel):
    ...
    def __init__(self, data, **kwargs):
        """override to parse the data"""
        super(CustomField, self).__init__(data, **kwargs)

        self.options = None
        if self.type == "labels":
            self.options = {
                option["id"]: option["label"] for option in self.type_config["options"]
            }
            if "value" in data.keys():
                self.value = [self.options[x] for x in self.value]
        elif self.type == "drop_down":
            self.options = {
                option["orderindex"]: option["name"]
                for option in self.type_config["options"]
            }
            if "value" in data.keys():
    ...
    def __repr__(self):
        """repr"""
        return "<{}.CustomField '{}'>".format(LIBRARY, self.name)
...
class Task(BaseModel):
    ...
    def __init__(self, data, **kwargs):
        ...
        self.custom_fields = {
            x['name']: CustomField(x, client=self._client) for x in self.custom_fields
        }
        ...
        self.time_estimate = (
            ts_to_timedelta(self.time_estimate) if self.time_estimate else None
        )
        self.waiting_on = [
            dep['depends_on'] for dep in self.dependencies if dep['task_id'] == self.id
        ]
        self.blocking = [
            dep['task_id'] for dep in self.dependencies if dep['depends_on'] == self.id
        ]
